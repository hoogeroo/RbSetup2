...
class MultiGoDialog(QDialog):
    ...
    def newsave(self):
        ...
        for p in ps:
          ...
          f.write(f'{paramname.replace(" ", "_")} {p.Startbox.value()} {p.Stopbox.value()} {p.time.currentIndex()} {p.channel.currentIndex()} {p.mytype.currentIndex()} {p.rampcheck.currentIndex()}\n')
        f.close()
    ...
    def newread(self,filename):
        ...
        if os.path.exists(filename):
            ...
            for i,l in enumerate(ls):
                ...
                try:
                    pars[i].rampCheck.currentIndex() # Params dont necessarily come with rampCheck - if it doesnt exist, we need to provide it!
                except:
                    setattr(pars[i],'rampCheck',QComboBox())
                try:
                    pars[i].rampCheck.setCurrentIndex(int(p[6].rstrip('\n'))) # If the mgo file doesnt have the rampCheck box, we'll default to false
                except:
                    pars[i].rampCheck.setCurrentIndex(0)
                #except:
                #    print('p[6].rstrip("\n") = '+p[6].rstrip("\n"))
                #    print()
...
class Parameters():
    ...
    class NewParam():
        def __init__(self):
            ...
            self.rampcheck=QComboBox()
            rampitems=['None','Start','Stop','Time Constant']
            for rampitem in rampitems:
              self.rampcheck.addItem(rampitem)
    ...
    def fixBoxes(self,myparam):
        ...
        if myparam.mytype.currentText()=="Time":
          ...
          temp=DAC_Box(0,0)
          Vmin = dac_conversions[myparam.channel.currentIndex()].minv#temp.minimum(); Vmax = temp.maximum(); stepsize = temp.singleStep()
          Vmax = dac_conversions[myparam.channel.currentIndex()].maxv
          stepsize = dac_conversions[myparam.channel.currentIndex()].step
    ...
    def fixAOMboxes(self,myparam):
      ...
      aom_num = int(myparam.channel.currentIndex()//2)#.astype(int)
      Vmin,Vmax,stepsize=0,0,1
      if dev[-4:]=="Freq":
        Vmin = 55; Vmax = 105
      elif dev[-4:]=="Ampl":
        Vmin = aom_amp_conversions[aom_num].minv
        Vmax = aom_amp_conversions[aom_num].maxv
        stepsize = aom_amp_conversions[aom_num].step
