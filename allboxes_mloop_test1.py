...
class allboxes(QWidget):
    ...
    def Optimise_pressed(self):
        if self.OptimiseButton.isChecked():
            ...
            print('Starting M-LOOP optimization...')

            # Get selected parameter file from dropdown
            index = self.cbParamSet.currentIndex()
            paramfile = self.cbParamSet.itemText(index).replace(' ', '-')
            mgo_path = os.path.join('/home/lab/mydata/Data/MLOOP_files', paramfile)

            # Load parameters
            self.mlpars = mg.Parameters(self.devicenames, self.stagenames, self.tek)
            self.mlpars.newread(mgo_path)
            self.mlparams = self.mlpars.params

            # Instantiate MLOOP interface
            self.optimiser = ML.MLOOPInterface(
                params=self.mlparams,
            ...
            def run_controller():
                try:
                    self.controller = mlc.create_controller(
                        interface=self.optimiser,
                        max_num_runs=self.sbMLNSteps.value(),
                        min_boundary=self.mlpars.min_boundary,
                        max_boundary=self.mlpars.max_boundary,
                        target_cost=0.0,
                        trust_region=True
                    )
                    self.controller.optimize()
                except Exception as e:
                    print(f"Error during M-LOOP optimization: {e}")
                finally:
                    # Cleanup after MLOOP finishes
            ...
            # Stop requested
            print("Stopping M-LOOP optimization...")
            if self.optimiser:
    ...
    def doGo(self, check_trap = False):
        ...
        if (nshots>0):
            ...
            if self.parent().princeton_action.isChecked():
              ...
              nmax,natoms,data=self.plotPrinceton(ns=nshots)
              self.camera.Natoms.append(natoms)
              self.camera.ODpeak.append(nmax)
    ...
    def plotPrinceton(self,ns=3):
        ...
        if self.fringeflag and (self.camera.nBGs>5):
          ...
          n_atoms,n_max=self.camera.getNatoms(mydata)
          #n_atoms = np.format_float_positional(n_atoms, precision = 3, unique = False, fractional = False, trim = 'k')
