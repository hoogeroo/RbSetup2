...
class ExptOptimiser(QWidget):
    ...
    def obtain_training_data(self, n_iterations = None):
        ...
        if n_todo <= 0:
            ...
            return False
        self.generate_params(self.params, N = n_iterations - self.niter)
        
        t_i = time.perf_counter()  
        for i in range(n_todo):
            self.parent().loadOptParams(self.params)#self.adjust_expt_params(params)
    ...
    def run_expt(self, params=None): 
        ...
        self.parent().saveOptData(self.foldstring)
    ...
    def load(self):
        ...
        name,path = QFileDialog.getOpenFileName(self, 'Open file','./models/',"Optimiser (*.txt)")
