...
class allboxes(QWidget):
    ...
    def __init__(self, tables, datapath=None, parent=None):#ndacs,naoms,ntimes):
      ...
      self.tableinit,self.curvesinit,self.eagledata, self.DIOinit,self.RFinit, self.MGinit,names, self.notes, headers = tables
      ...
      # ALL NAMES HERE LOADED IN BY IMPORTING 'dtry:
      ...
      self.make_jpg = True
      print("got out of allboxed init")
...
#      self.layout.addWidget(self.plotSet,0,2+maxcol,len(stge.DC)+3,1)
    ...
    def makeDCLabels(self,initial, stge):#, layout = None)The file “~/artiq/artiq-env/FastWindow.py” changed on disk.:
    ...
    def makeRFwindow(self,initial=False):#, layout = None):
      ...
      if initial:
        print("Hello")
        self.tek=afg.TekAFG(self)#parent=self)
        print("TekAFG exit")
    ...
    def loadData(self):
        ...
        for tid in range(len(stages)):
            ...
            #print
            #print(f"leng {len(ones)} for timesteps {timesteps}")
            ...
            for row in range(len(self.devicenames['DAC'])):
                ...
                #ones_list.append(ones_val_array)
            #print(box_vals)
            stage_list.append([box_vals,timesteps])
        self.data=data
        ...
        with open("myrun.py", "w") as a:
            ...
            a.write("        self.timesteps = [stage[1] for stage in stages]\n")
            a.write("    @kernel\n")
            a.write("    def record(self):\n")
            a.write("        with self.core_dma.record(\"test\"):\n")
            a.write("            for i in range(len(self.voltage_arrays)):\n")
            a.write("                voltages = self.voltage_arrays[i]\n")
            a.write("                timestep = self.timesteps[i]\n")
            a.write("                with parallel:\n")
            a.write("                  for channel in range(len(voltages)):\n")
            a.write("                      self.fastino0.set_dac(channel, voltages[channel])\n")
            a.write(f"                      delay({self.timestep} * ms)\n")           
            a.write("                delay(timestep * ms)\n")           
            #a.write(f"                  delay({self.timestep} * ms)\n")
            #a.write('                  print("Timestep:", timestep, "Current time:", now_mu())\n')
            #a.write(f"                  delay({self.timestep} * ms)\n")
            a.write("    @kernel\n")
            a.write("    def run(self):\n")
            a.write("        self.core.reset()\n")
            a.write("        self.record()\n")
            a.write("        test = self.core_dma.get_handle(\"test\")\n")
            a.write("        self.core.break_realtime()\n")
            #a.write("        for i in range(len(self.voltage_arrays)):\n")
            #a.write("            self.core_dma.playback_handle(test)\n")
            a.write("        self.core_dma.playback_handle(test)\n")
            #a.write(f"            delay({self.timestep} * ms)\n")

            a.flush() #not needed as for loop closes
            a.close() #same, but no point deleting for now
            #a.write(f"          t = {box_val} * np.ones({timesteps})\n")

            #a.write(f"          for i in range(len{}):\n")
            #a.write(f"              self.fastino0.set_dac({row},t[i])\n")
            
                    #string= "              delay("+str(self.timestep)+"*ms)\n"
       # print("gap")
        #print(stage_list,'box_val_list and length >',len(stage_list))
        #print("box len",#len(box_val_array))
        #print("ones_length",len(ones_list))
        #ones_ones = []
        #for i in range(len(ones_list)):
        #    ones_ones.append(ones_list[i][0])
        #print(ones_ones,"ones_ones")
        #print("ones first", ones_list[0])
        # End of DAC loop
