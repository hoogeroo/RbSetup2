    ...
    def record(self):
        with self.core_dma.record("test"):
            for i in range(len(self.voltage_arrays)):
                ...
                delay(20*ms)
                self.urukul1_ch0.set(frequency = int(ch0_freq)*MHz, amplitude = ch0_amp)
                self.urukul1_ch1.set(frequency = int(ch1_freq)*MHz, amplitude = ch1_amp)
                self.urukul1_ch2.set(frequency = int(ch2_freq)*MHz, amplitude = ch2_amp)
                self.urukul1_ch3.set(frequency = int(ch3_freq)*MHz, amplitude = ch3_amp)
                #print(array)
                #delay(1000*ms)
                #print(channels[0])
