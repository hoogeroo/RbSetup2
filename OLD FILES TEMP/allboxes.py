...
class allboxes(QWidget):
    ...
    def change_RF_stage(self):
        ...
        print(index)
        newvalue=self.Timeboxes[index[0]].value()
...
#          self.DCSpinboxes[20].setValue(0.0)
   ...
   #         return(-999)
       ...
       for i in range(npts):
         ...
         if (eid<3):
             ...
             for p in mgpars:
                 ...
                 #e_val=float(p.e_val)
                 if npts>1:
                     myvalue=p.b_val+i*(p.e_val-p.b_val)/(npts-1)
                 else:
                     myvalue=p.b_val
                 tid=p.time
                 cid=p.channel
    ...
    def AddCol(self):
        ...
        self.stagelabels.append(QLabel('Stage '+str(self.ntimes+1)))
