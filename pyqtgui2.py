...
#['Add Column on Right','Undo','Redo', 'Insert Column before Cursor', 'Remove Column at Cursor'],[self.CW.AddCol,None, None, None, None])            
        ...
        mg_plot_option_menu=option_menu.addMenu("MultiGo Plot Options")
        self.mg_plot_option_nmax=QAction('Max density',checkable=True)
        self.mg_plot_option_natoms=QAction('Number of Atoms', checkable=True)
        mg_plot_option_menu.addAction(self.mg_plot_option_natoms)
        mg_plot_option_menu.addAction(self.mg_plot_option_nmax)
        self.mg_plot_option_nmax.setChecked(True)
        self.mg_plot_option_natoms.triggered.connect(self.mg_plot_option_natoms_function)
        self.mg_plot_option_nmax.triggered.connect(self.mg_plot_option_nmax_function)
        self.CW.mg_plot_option=1
        ...
        self.lowpass_action=QAction('Low pass on image',checkable=True)
        option_menu.addAction(self.lowpass_action)
        self.lowpass_action.triggered.connect(self.lowpassfunc)
    ...
    def mg_plot_option_natoms_function(self):
      if self.mg_plot_option_natoms.isChecked():
        ...
        self.CW.mg_plot_option=2
    def mg_plot_option_nmax_function(self):
      if self.mg_plot_option_nmax.isChecked():
        self.mg_plot_option_natoms.setChecked(False)
        self.CW.mg_plot_option=1
    ...
    def load_blackfly(self, headers):
        try:
          self.bfcam.dims[0] = headers['BF_DIM_0']
          self.bfcam.dims[1] = headers['BF_DIM_1']
          self.bfcam.center[0] = headers['BF_CEN_0']
          self.bfcam.center[1] = headers['BF_CEN_1']
        except: # The below are used on old .fits files but not recently-saved ones.
          self.bfcam.dims[0] = headers['BF_DIMS_0']
          self.bfcam.dims[1] = headers['BF_DIMS_1']
          self.bfcam.center[0] = headers['BF_CENTER_0']
          self.bfcam.center[1] = headers['BF_CENTER_1']
        self.bfcam.roi = self.bfcam.get_roi()#[int(self.bfcam.center[i]+a*self.bfcam.dims[i]) for i in range(2) for a in [-0.5, 0.5]]
        if self.bfcam.connected:
          self.bfcam.change_roi()
          #print(self.bfcam.dims, self.bfcam.center)
        #except:
          #print('Blackfly figure dimensions/center information not found in file - using default values instead')
        
        if hasattr(self,'bfcam'):
            print(f'Blackfly cam loaded! (self.bfcam = {self.bfcam})')
...
#==============================================================================================================
    def saveasaction(self): # Action for file save
        ...
        path='/home/rabi/lab/mydata/Data/'+mydate
...
#==============================================================================================================
#==============================================================================================================
    def mysave(self,filename, make_jpg = False):
    ...
    #\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/          
        ...
        prihdr['BF_CEN_0'] = 550#(self.bfcam.center[0], 'Blackfly Figure Center row')
        prihdr['BF_DIM_0'] = 500#(self.bfcam.dims[0], 'Blackfly Figure Dimensions row')
        prihdr['BF_CEN_1'] = 1050#(self.bfcam.center[1], 'Blackfly Figure Center column')
        prihdr['BF_DIM_1'] = 500#(self.bfcam.dims[1], 'Blackfly Figure Dimensions column')
        prihdr['FLUO_Z']   = (self.CW.fluo_zero, 'Zero Fluorescence')
        prihdr['FLUO_M']   = (self.CW.fluo_max, 'Max Fluorescence')
        prihdr['FLUO_P']   = (self.CW.fluo_percentage, 'Fluorescence percentage')
    #\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
        ...
        for i in range(len(RFstate)):
            tabhdr.append((f'RF{i}', RFstate[i]))
        for j in range(len(MGstate)):
            tabhdr.append((f'MG{j}', MGstate[j]))
        for k in range(len(names)):
            tabhdr.append((f'HIERARCH {names[k]}', notes[k]))
    ...
    #\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/
        try:
            ...
            try:
              ...
              self.bfcam.center[0] = headers['BF_CEN_0']
              self.bfcam.center[1] = headers['BF_CEN_1']
              self.bfcam.roi=[int(self.bfcam.center[i]+a*self.bfcam.dims[i]) for i in range(2) for a in [-0.5, 0.5]]
              print(self.bfcam.dims, self.bfcam.center)
            except:
              print('Blackfly figure dimensions/center information not found in file - using default values instead')
            try:
              self.CW.fluo_zero= headers['FLUO_Z']
              self.CW.fluo_max=headers['FLUO_M']
              self.CW.fluo_percentage=headers['FLUO_P']
            except:
              print('Fluo pars not found, using default values')
...
class BlackflyOptionDialog(QDialog):
    ...
    def make_dialog(self):
        ...
        self.contourflag=QCheckBox()
        self.contourflag.setTristate(False)
        self.contourflag.setChecked(self.bf.contourflag)
        self.layout.addWidget(QLabel('Plot contours?:'),5,0)
        self.layout.addWidget(self.contourflag,5,1)

        self.changeValsButton=QPushButton('Change Values')
        self.changeValsButton.clicked.connect(self.changeBFValues)
        self.layout.addWidget(self.changeValsButton,6,0)
        
        self.setLayout(self.layout)
        
    def changeBFValues(self):
        self.bf.roi=[box.value() for box in self.roiboxes]
        if self.bf.connected:
            self.bf.change_roi()
        
        print(f'self.filterflag.checkState()={self.filterflag.checkState()}')
        if self.filterflag.isChecked():
            self.bf.lowpassflag=True
        else:
            self.bf.lowpassflag=False
        print(f'self.bf.lowpassflag={self.bf.lowpassflag}')
                
        print(f'self.contourflag.checkState()={self.contourflag.checkState()}')
        if self.contourflag.isChecked():
            self.bf.contourflag=True
        else:
            self.bf.contourflag=False
        print(f'self.bf.contourflag={self.bf.contourflag}')
