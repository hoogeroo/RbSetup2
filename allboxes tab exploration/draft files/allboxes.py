...
class allboxes(QWidget):
    ...
    def GoAction(self, check_trap = False):
        ...
        wait(1.6) # Need at least 1sec before blackfly ready for pictures; I'm trying to keep this as short as possible while still working
        self.pulse_off()        
        wait(0.2)
        #try:
        #    s.connect((TCP_IP2, TCP_PORT))
        ...
        if (nshots>0):
            if self.parent().blackfly_action.isChecked():
              ...
              if result is not None:
                self.BGimages, bg_resetflag = result
                if bg_resetflag:
                  self.nBGstored = 1
                else:
                  self.nBGstored+=1
