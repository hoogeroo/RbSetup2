...
class DC_DAC_Box(DAC_Box):
    ...
    def updateDAC(self):
        self.updatecolor()
        import subprocess
        artfile=open("updatedac.py","w")
        artfile.write("from artiq.experiment import *\n")
        artfile.write("class ChangeDAC(EnvExperiment):\n")
        artfile.write("   def build(self):\n")
        artfile.write("      self.setattr_device('core')\n")
        artfile.write("      self.setattr_device('fastino0')\n")
        artfile.write("   @kernel\n")
        artfile.write("   def run(self):\n")
        artfile.write("      self.core.reset()\n")
        artfile.write("      delay(1e-3)\n")
        string="      self.fastino0.set_dac("+str(self.DACid)+","+str(self.value())+")\n"
        artfile.write(string)
        artfile.write("      delay(1e-3)\n")
        artfile.flush()
        artfile.close()
