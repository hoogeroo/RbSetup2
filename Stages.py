...
class Stages():
        ...
        for i in range(len(devices)):
          ...
          if not type(DIOstate)=='NoneType':
            try:
              ...
              self.DC[-1].setChecked(False)
      elif devtype=='AOM':
        for i in range(len(devices)):
            j=i//2
            if not type(vals)=='NoneType' and n+i < len(vals):
                if not i%2:
                    try:
                      self.DC.append(DC_AOM_Freq_Box(j,1,val=vals[n+i]))
                    except:
                      print(f'DC append failed - len(vals) = {len(vals)}, n_dacs + i = {n+i}')
                      self.DC.append(DC_AOM_Freq_Box(j,1))
                else:
                    try:
                      self.DC.append(DC_AOM_Ampl_Box(j,1,val=vals[n+i]))
                    except:
                      print(f'DC append failed - len(vals) = {len(vals)}, n_dacs + i = {n+i}')
                      self.DC.append(DC_AOM_Ampl_Box(j,1))
            else:
                if not i%2:
                    self.DC.append(DC_AOM_Freq_Box(j,1))
                else:
  ...
  def addStage(self,time=0,name=None,vals=None,DIOstate=None,position='last'):
    ...
    for i in range(len(stagevals)):
      if isinstance(stagevals[i], QCheckBox):
        ...
        if vals[valcount]<-25:
          stagevals[i].setRange(-30,30)
          stagevals[i].setValue(-30)
        elif vals[valcount]<-15:
          stagevals[i].setRange(-20,20)
          stagevals[i].setValue(-20)
          print(vals[valcount])
        else:
          stagevals[i].setValue(vals[valcount])
        valcount+=1
      if valcount == len(vals):
        break
  ...
  def populate(self, table,curves,DIOstate=None,names=None):
    ...
    if len(curves.names)>1:
      for k,curvename in enumerate(curves.names): # Because we're working with .fits fields, we want to iterate through fields
                                                  # Each field corresponds to one ramp, which are each associated with a name.
                                                  # Iterating through names and using those to draw data from each 'field' is the easy approach
                                                  #   See: https://docs.astropy.org/en/stable/io/fits/index.html#working-with-table-data
        
        name=curvename.split()
        vals = curves.field(k)
      ...
      namestr = curves.names[0] # Hopefully this works fine? Not sure if names are a list or a str when only one entry
      name=namestr.split()
      vals = curves.field(0) # This should work the same as above
  ...
  def store(self):
    ...
    for i in range(len(self.stages)):
      ...
      #colname='0'+
      if self.stages[i].enabled:
        colname='1'+self.stages[i].name
      else:
        colname='0'+self.stages[i].name
      columns.append(fits.Column(colname, array=np.array(col), format='E'))
...
class newStage():
  def __init__(self,name,stageid, t,devdict):
    ...
    if name[0]=='0':
      ...
      #print('Not enabled',stageid,name)
    else:
      self.enabled=True
    for devtype, devices in devdict.items():
      if devtype=='DAC':
        ...
        for i in range(len(devices)):
            if not i%2:
                self.boxes.append(AOM_Freq_Box(i//2,stageid))#i+len(devdict['DAC']),stageid))
            else:
                self.boxes.append(AOM_Ampl_Box(i//2,stageid))#1+i+len(devdict['DAC']),stageid))
      elif devtype != 'EagleDAC':
        Exception('Too many different device types!')
      if not self.enabled:
