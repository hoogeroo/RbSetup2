...
def removefringe(foregrounds,backgrounds,bgmask):#fringeremoval1(foregrounds,backgrounds,bgmask):
    ...
    R=backgrounds.reshape([sx*sy,len(backgrounds)])#nimgs]) 
    ...
    for j in range(nimgs):
        Akj=A[k,j].reshape(nk) #extract background-mask pixels from foreground shots, shape into column vec for upcoming dot product
...
def filter_mask2(z):
    ...
    for i in [-1,0,1]:
        for j in [-1,0,1]:
                    # I've rewritten this to be agnostic re: the size of f in x or y
                    out+=0.1*(1+(i==j and not i))*z[1+i:na-1+i,1+j:nb-1+j] # np.size(f)*mean(matrix) is just a quick way of writing sum(sum(...))
    return out 
