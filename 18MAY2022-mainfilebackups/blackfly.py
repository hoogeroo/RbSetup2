...
class blackfly():
  def __init__(self):
    ...
    self.dims = [400, 400]#[400,600] # y/rows, x/cols
    self.center= [730,940]#[880,1150] # y/rows, x/cols
    self.roi=[int(self.center[i]+a*self.dims[i]) for i in range(2) for a in [-0.5, 0.5]]
    self.data=np.zeros((3,self.dims[0]-2*self.lowpassflag,self.dims[1]-2*self.lowpassflag))
  ...
  def show(self,nshots,BGimages,nBG):  
    ...
    if nshots==3:
      ...
      if self.lowpassflag:
        cutout=klib.filter_mask2(pic[self.roi[0]:self.roi[1],self.roi[2]:self.roi[3]])
      else:
        cutout=pic[self.roi[0]:self.roi[1],self.roi[2]:self.roi[3]]
      try:
        global cutout212
        if self.lowpassflag:
          cutout212=klib.filter_mask2(pic2[self.roi[0]:self.roi[1],self.roi[2]:self.roi[3]])
        else:
          cutout212=pic2[self.roi[0]:self.roi[1],self.roi[2]:self.roi[3]]
      except:
        pass
      
      self.data[0,:,:]=cutout[:,:]
      self.data[1,:,:]=cutout212[:,:]
      ...
      if self.fitflag:
          ...
          if sum([x==None for x in [height,wx,wy]])>0:
  ...
  def plot_fringeremoval(self,fs,nshots,BGimages,nBG):
      ...
      for i in range(nshots):#i, pic in enumerate([pAtom,pRef,BG]):
          pic=self.readfile(fs[i-nshots]) # patom = fs[-3]; pRef=fs[-2]; no-light background BG = fs[-1]
          if self.lowpassflag:
              cutout[i,:,:]=klib.filter_mask2(pic[self.roi[0]:self.roi[1],self.roi[2]:self.roi[3]])
          else:
              cutout[i,:,:]=pic[self.roi[0]:self.roi[1],self.roi[2]:self.roi[3]]
          self.data[i,:,:]=cutout[i,:,:]
      ...
      #print(cutout[0,:,:]-pic0)
      #print(cutout[1,:,:]-pic0)
      BGimages[nBG%len(BGimages)]=cutout[1,:,:]-pic0
      ...
      N_atoms = 0
      if nBG>1000 and nshots==3:
          finalindex=min(len(BGimages)-1,nBG)
          bgmask = klib.prepare_bgmask(dims=self.dims)
          mydiff,optrefimage,tel = klib.removefringe(cutout[0,:,:]-pic0+1,[bg+1 for bg in BGimages[:finalindex]],bgmask)
      elif nshots>1:
          mydiff=-np.log((np.abs(cutout[0,:,:] - pic0)+1)/(np.abs(cutout[1,:,:] - pic0)+1))
      ...
      if self.fitflag:
          ...
          # For gaussian, assume particle number ~ int Gauss dx dy = w_x * w_y * height
              ...
              plt.title("Width = {}x, {}y; N_est = {}".format(roundstr(wx,0),roundstr(wy,0),N_est))#,roundstr(wy*py[0]/1000,0)))
          except:
              plt.title=("Fits failed")
      else:
          print('Gaussian fits currently disabled - see \'self.fitflag\' in blackfly.py')
