...
class allboxes(QWidget):
    ...
    def makeGOLOADbuttons(self,initial=False):
      if initial:
        ...
        self.fluo_zero = 140 # Use in future to set zero point
        self.fluo_max = 500
        self.fluo_percentage = 0.9 # Must be number between 0 and 1
        self.fluo_timeout = 20 # units: seconds
    ...
    # TO DO
      ...
      if self.optimiser is None:
        ...
        if not self.optimiser.model_location.split('/')[-1] == self.cbParamSet.currentText():
          self.MLParamSetAction()
    ...
    def GoAction(self, check_trap = False):
        ...
        if self.cbMOTLoad.isChecked() or self.MG_active:
          self.pulse_on()
    ...
    def MOTTimerAction(self):
          self.fluorescence=self.adc.read(0)
...
class FluoDialog(QDialog):
    def __init__(self, zero, maxval, threshold_percentage, timeout, sampletime, adc):#filename,devnames,stagenames, tek):
        ...
        self.mins = [1,1,0.01, 1, 0.05]
        self.maxes = [9999,9999,1,60,10]
        self.decimals = [0,0,2,0,2]
