...
class blackfly():
  def __init__(self):
    ...
    self.dims = [500,500]#[1000, 1000]#[400,600] # y/rows, x/cols
    self.center= [550,1200]#[880,1150] # y/rows, x/cols
  ...
  def plot_fringeremoval(self,fs,nshots,BGimages,nBG):
      ...
      if nBG>10 and nshots==3:
          ...
          mydiff,optrefimage,tel = klib.removefringe(tempFG+.1,BGset,bgmask)
      
      elif nshots>1:
          #top = np.abs(cutout[0,:,:] - pic0)
          #bottom = np.abs(cutout[1,:,:] - pic0)
          #cornermask = klib.prepare_bgmask(dims=pic_nocrop[0].shape, bgcor = 10)
          cornermask = np.full(pic_nocrop[0].shape, False)
          cornermask[self.roi[0]:self.roi[1],self.roi[2]:self.roi[3]] = True
          cornermask[self.roi[0]+10:self.roi[1]-10,self.roi[2]+10:self.roi[3]-10] = False
          
          top_nocrop = np.abs(pic_nocrop[0] - pic_nocrop[2]); bottom_nocrop = np.abs(pic_nocrop[1] - pic_nocrop[2])
          top_nocrop[pic_nocrop[0] < pic_nocrop[2]] = 0; bottom_nocrop[pic_nocrop[1] < pic_nocrop[2]] = 0; 
          norm_ratio = np.mean(bottom_nocrop[cornermask]+.1)/np.mean(top_nocrop[cornermask]+.1)
          #norm_ratio = np.mean(np.amin(bottom,axis=0)) / np.mean(np.amin(top,axis=0))
          print(f'norm_ratio = {norm_ratio}; FG mean = {np.mean(top_nocrop[cornermask])}; BG mean = {np.mean(bottom_nocrop[cornermask])}')
          
          top = np.abs(tempFG); bottom = np.abs(tempBG)
      ...
      if self.fitflag:
          try:
            prefix = ''
            height,wx,wy, contourdata=self.gaussfit(mydiff, contourflag=True)
  ...
  def gaussfit(self,pic, fittype = '2d',contourflag=False):
    ...
    if contourflag:
        ...
        results = [height,width[0],width[1]]
