...
class Parameters():
    def __init__(self,devnames,stagenames):
        self.params=[]
    ...
    def newpar(self):
        ...
        self.params.mytype.setCurrentIndex(0)
        self.params.time.setCurrentIndex(0)
        self.params.channel.setCurrentIndex(0)
        load_devices_stages(self.params[-1])
        ...
        self.params[-1].mytype.currentIndexChanged.connect(self.load_devices_stages,self.params[-1])
    ...
    def update_devices(self,myparam):
        ...
        if not curtext == 'Time' and not curtext == 'RF (freq)' and not curtext == 'RF (amp)':
    ...
    def newsave(self):
        ...
        for i in range(np):
          ...
          f.write("%d " % self.params[i].time.currentIndex())
          f.write("%d " % self.params[i].channel.currentIndex())
        ...
        if os.path.exists(filename):
            ...
            for i,l in enumerate(ls):
                ...
                try:
                  ...
                  self.params[i].time.setCurrentIndex(int(p[3]))
                  self.params[i].channel.setCurrentIndex(int(p[4]))
                except:
                  pass
            if len(ls) < npars:
...
class MultiGoDialog(QDialog):
    def __init__(self, filename,devnames,stagenames):
        super(QDialog, self).__init__()
        self.setWindowTitle("MultiGo Parameters")
        self.layout = QGridLayout()
        self.params=Parameters(devnames,stagenames)
        #self.devnames=devnames
        #self.stagenames=stagenames
        ...
        self.addParButton.clicked.connect(self.params.newpar)
        self.delParButton.clicked.connect(self.params.delpar)
        self.layout.addWidget(self.addParButton,0,1)
        self.layout.addWidget(self.delParButton,0,2)
        QBtn = QDialogButtonBox.Ok | QDialogButtonBox.Cancel
        self.buttonBox = QDialogButtonBox(QBtn)
        self.buttonBox.accepted.connect(self.params.naccept)
        self.buttonBox.rejected.connect(self.params.reject)
        ...
        self.loadButton.clicked.connect(self.params.newread2)
        self.saveButton.clicked.connect(self.params.newsave)
