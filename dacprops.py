...
#percentage = np.array([0, 3, 4, 5, 6, 7, 8, 9, 10, 20, 30, 40, 50, 60])
#dipole_powers = np.array([0,23.3E-3, 59E-3, 0.165, 0.377, 0.715, 1.18, 1.79, 2.51, 14.6, 31, 49.3, 65.4, 71.1]) # in milliWatts

## New calibration 11/03/25
percentage = np.array([0, 1, 2, 3, 4, 5, 10, 20, 30, 40, 50, 60, 70])
dipole_powers = np.array([0, 1.7E-3, 5.7E-3, 30.5E-3, 0.133, 0.425, 6.6, 38.0, 80.9, 128, 172, 203, 215]) # in milliWatts
dipole_powers *= power_max/max(dipole_powers)
dipole_volts = 3.4 * percentage/max(percentage) # in Volts
dipole_pars = np.polyfit(dipole_powers, dipole_volts, 5)#We want to put in a desired power and get back a voltage

#dac_conversions[1].pars=np.array([0])
