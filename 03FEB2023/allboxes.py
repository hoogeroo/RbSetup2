...
class allboxes(QWidget):
    ...
    def makeMLbuttons(self,initial=False):
      if initial:
        ...
        self.MLLoadButton.clicked.connect(self.MLLoadAction)
    ...
    def makeGOLOADbuttons(self,initial=False):
      if initial:
        ...
        self.GOSaveLabel=QLabel('Save \'Go\' images?')
      ...
      if self.optimiser is None:
        ...
        if not setmatch:
          ...
          if answer == msg.Abort:
            ...
            self.optimiser.create_folders(filename=fname)
    ...
    # TO DO
        if load_from_file:
          fname,path = QFileDialog.getOpenFileName(self, 'Open file','./models/',"Optimiser (*.mgo)")
          fname = fname[:-4]
        else:
          fname=self.cbParamSet.currentText()
        
        mlpars = mg.Parameters(self.devicenames,self.stagenames,self.tek)
        mlpars.newread(fname+'.mgo')
        if not self.optimiser:
          trainingsteps = self.sbMLNSteps.value()
          self.optimiser = ML.ExptOptimiser(mlpars.params, trainingsteps = trainingsteps, filename = fname, parent=self)
        
        if os.path.isfile(fname+'.pkl'):
          self.optimiser.load(mlpars.params, name=fname+'.pkl')
        else:
          print(f'No prior model for {fname[:-4]}.mgo found. Starting fresh!')
