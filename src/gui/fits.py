...
def load_settings(path, window):
    ...
    if images is not None:
        window.plots_gui.update_images(CameraImages(images[0].astype(np.float32), images[1].astype(np.float32), images[2].astype(np.float32), images[3].astype(np.float32) * 10000))
